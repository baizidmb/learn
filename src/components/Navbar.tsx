...
export const Navbar: React.FC = () => {
  ...
  const navItems = [
    ...
    { id: 'glossary', label: 'Glosar', labelEn: 'Glossary', icon: BookOpen, count: stats.glossaryTotal },
    ...
    <header className="sticky top-0 z-40 bg-slate-950/90 backdrop-blur-md border-b border-slate-800/80 shadow-xl">
      <div className="max-w-md mx-auto px-4 py-2.5 flex items-center justify-between">
        ...
        <button 
          ...
          className="flex items-center gap-2.5 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-paprika-500 rounded-xl p-1 transition-transform active:scale-95"
          aria-label="Acasă - Marissa RO-EN"
        >
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-paprika-500 via-brass-500 to-paprika-600 p-0.5 shadow-md">
          ...
          <div>
            <h1 className="font-display text-xl leading-none tracking-wider text-ticket-paper uppercase font-extrabold flex items-center gap-1">
              MARISSA <span className="text-paprika-500 font-mono text-xs px-1.5 py-0.5 rounded bg-paprika-950 border border-paprika-700/50">RO⇄EN</span>
            </h1>
            <p className="text-[10px] text-slate-400 font-mono tracking-tight flex items-center gap-1 mt-0.5">
              <span>Kitchen & Restaurant Staff</span>
              <Sparkles className="w-2.5 h-2.5 text-brass-400" />
            </p>
          </div>
        </button>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Streak Badge */}
          <div 
            className="flex items-center gap-1 bg-gradient-to-r from-paprika-950 to-slate-900 border border-paprika-700/60 text-paprika-400 px-2.5 py-1 rounded-full text-xs font-mono font-bold shadow-md"
            title={`${streakDays} days learning streak!`}
          >
            <Flame className="w-3.5 h-3.5 text-paprika-500 fill-paprika-500 animate-bounce motion-reduce:animate-none" />
      ...
      <nav className="max-w-md mx-auto px-2 flex justify-around border-t border-slate-800/70 bg-slate-950/95">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex-1 py-2 px-1 flex flex-col items-center gap-0.5 transition-all text-xs font-medium min-h-[50px] justify-center relative ${
                isActive
                  ? 'text-paprika-400 bg-paprika-950/40 font-bold'
                  : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/60'
              }`}
              aria-label={item.labelEn}
            >
              {isActive && (
                <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-gradient-to-r from-paprika-500 to-brass-400 rounded-full" />
              )}
              <Icon className={`w-4 h-4 ${isActive ? 'scale-110 text-paprika-500' : ''} transition-transform`} />
